...
describe('KiwiNamedItem', () => {
	...
	it('Throws error when getting non-existent name', async () => {
		...
		expect(KiwiNamedItem.getByName('Bob')).rejects.toThrowError('KiwiNamedItem with name "Bob" could not be found.');
	});
	...
	it('Can get multiple KiwiNamedItem by ID array', async () => {
		const serverItems = [{ id: 1, name: 'Bob', otherKey: 'otherVal'}, { id: 2, name: 'Stuart', otherKey: 'otherOtherVal'}];
		mockAxios.post.mockResolvedValue(mockRpcResponse({result: serverItems}));
