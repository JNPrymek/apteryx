...
import { mockNetworkSuccessResponse } from '../../test/networkMocks/mockNetworkResponse';
...
describe('HTTP Request Handler', () => {
	...
	it('Can send a POST request with provided data and get response data', async () => {
		...
		expect(response).toEqual({
			...
			headers: {
				...
				'set-cookie': 'resCustomCookie=abcdefg; expires=Sat, 30-Dec-2221 06:12:11 GMT; path=/; domain=example.com',
			},
		});
	});
	...
	describe('Save Cookies to Jar', () => {
		...
		it('Can save single cookie string to jar', async () => {
			await RequestHandler.saveCookiesFromHeader(serverUrl, { 'set-cookie': cookieA.cookieString() });
	...
	it('Can send a POST request with cookie and get response data', async () => {
		// Prep Test Cookies
